...
```bash
git -C core checkout -- media/media-jvm/src/main/java/com/omnimusic/media_jvm/mpv/MpvPlayerAdapter.kt
rm -rf core/media/media-jvm/src/main/java/com/omnimusic/media_jvm/memory
...
```bash
pid=$(pgrep -f -i omnimusic | while read p; do [ "$(cat /proc/$p/comm)" = omnimusic ] && echo $p; done | head -1)
...
```powershell
Get-Process OmniMusic | Select-Object Name,Id,@{n="WorkingSet_MB";e={[int]($_.WorkingSet64/1MB)}}
jcmd <pid> GC.heap_info
```
