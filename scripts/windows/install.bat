@echo off
REM OmniMusic offline installer for Windows.
...
REM   - omnimusic.crt
REM   - omnimusic-<version>.msix     (any .msix matching omnimusic-*.msix)
REM
...
echo.
echo === OmniMusic offline installer ===
echo.

REM --- 2. Install cert to Trusted People (idempotent via certutil) -----------
if not exist "%~dp0omnimusic.crt" (
    echo [ERROR] omnimusic.crt not found next to install.bat
    pause
    exit /b 1
)

echo [1/3] Importing omnimusic.crt to LocalMachine\TrustedPeople ...
certutil -addstore -f "TrustedPeople" "%~dp0omnimusic.crt"
if %errorlevel% neq 0 (
...
set "MSIX="
for %%f in ("%~dp0omnimusic-*.msix") do set "MSIX=%%f"

if not defined MSIX (
    echo [ERROR] No omnimusic-*.msix found in this folder.
    pause
    exit /b 1
)
...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    "try { Add-AppxPackage -Path '%MSIX%' -ForceApplicationShutdown -ForceUpdateFromAnyVersion -ErrorAction Stop; exit 0 } catch { Write-Host 'In-place update failed; removing existing OmniMusic and reinstalling...'; Get-AppxPackage -Name 'OmniMusic' | Remove-AppxPackage -ErrorAction SilentlyContinue; try { Add-AppxPackage -Path '%MSIX%' -ForceApplicationShutdown -ErrorAction Stop; exit 0 } catch { Write-Host $_; exit 1 } }"
...
REM --- 4. Launch app ---------------------------------------------------------
echo.
echo [3/3] Launching OmniMusic ...
start "" "shell:appsFolder\OmniMusic_ejp2bhxmz1qq6!OmniMusic"

echo.
echo OmniMusic installed successfully.
echo To uninstall: Settings -^> Apps -^> Installed apps -^> OmniMusic -^> Uninstall
echo.
pause
endlocal
